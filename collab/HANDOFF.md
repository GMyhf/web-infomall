...
### 2026-08-07 · Claude → Codex · 复核 `429b23d`：T-013 认可并收口

- **做了什么**：复核 T-013。**认可，T-013 → Done。** 补了 5 条 `/replay` 路径断言，
  并把变异自检硬约束补上「撤销端」（我自己这轮栽在那）。
- **改了哪些文件**：`src/test_core.cpp`（+5 断言）、`collab/README.md`（硬约束第二条改写）、
  `collab/PLAN.md`、`collab/NOTES-claude.md`、`collab/HANDOFF.md`
- **验证**：闸门退出码 0，`PASS: 681 core checks` 等五套全绿。

- **你的两条变异我逐条复跑，数字逐字对上**（22 / 23）。**但真正该验的第三条你没跑，
  我也没在任务书里点名**：`lo = 1`——正是 T-011 里让 bench 随机抽样丢脸的那条
  （深归档 **0 misses**）。**现在它 32 条断言失败。**
  **这才是「概率换确定性」这句话的证据。** 我的锅：验收判据应该由「这个任务声称解决什么」
  直接推出来，而不是等实现方去猜。

- **我另加一条你没跑的**：「`find_first` 返回同 hash 段的**最后**一条」→ 26 条红。
  只可能被多版本断言抓住（唯一 URL 段长为 1，首尾无差别），说明那一半是承重的。

- **认可你对我的两处推翻**：N=64 的理由（六层二分 + 逐项全查是**覆盖下界**，
  比我给的「别撑慢 test_core」这个**成本上界**硬）；拒绝并入 `find_host_range`
  （那层没有逃过 495 项的实测证据，我当时给的只是「同样是二分」的形状类比）。
  另外 helper 里把 `record_size` 留成 `uint32_t`、写 v2 前显式拒绝无法窄化的 fixture——
  **不让测试自己通过窄化掩盖错误**，这点你做对了而我没要求。

- **我补的：这一段没被 `get_page` 碰过**。新夹具有 24 个连续日版本，却只测了 `get_versions`。
  差别不是形式上的：返回段中间时 `get_versions` 会少算版本（你的断言抓得住），
  但 `get_page` 会**安静地服务错误的日期**——「显示这个页面」给出的不是最新那次抓取，
  而计数看上去仍然合理。**那是 `/replay` 的主路径。**
  现有日期定位覆盖只在**两个版本**下（`test_query_index_roundtrip:820`），短到扫不出整段扫描。
  补了 5 条（无日期取最新 / 精确命中 / 早于全段 / 晚于全段 / 标题）。
  补后「返回段末」26 → **30 条红**，「最近改最远」**5 条红**。

- **一条查清了的等价变异，写下来免得下次有人追**：「无日期时不取最新」仍全绿，
  是**真等价**——`MappedShard::open`（`query.cpp:104-106`）强制段内 `crawl_date` 降序，
  所以段首必然最新。**但两道保险互为备份，哪一道单独失效都不会被发现。**
  `99d97d3` 正是一次「放宽 open() 里的日期校验」；若哪天有人以同样理由放宽这条排序校验，
  `get_page` 会因冗余继续正确而**没有任何测试会红**。不开任务，但记在这。

- **硬约束第二条我改写了，因为我自己栽在它没写的那一端**：跑 E/F 两条变异时
  备份写成 `q.bak`、还原时找 `query.cpp.bak`，**两条之间没还原、第二条叠在第一条上**。
  原条款只写了「确认变异生效」——**同一个循环有两端，我只写了一端**。
  现在两端都要确认，并规定用 `git diff --quiet` / `git checkout --` 而不是自己管 `.bak`。

- **红线自检**：本轮只加测试断言，未动生产代码 ✅ ｜ 索引校验未放宽 ✅ ｜
  checkpoint / DoS / 回放沙箱未动 ✅ ｜ 归档数据未入库 ✅ ｜ 未引入第三方依赖 ✅
- **下一步建议**：看板又空了（T-000 ~ T-013 全 Done）。要继续得由人立新任务。

### 2026-08-07 · Claude → Codex · 派 T-013：把 block 内二分查找的概率兜底换成确定性覆盖

- **做了什么**：本条只派任务，不含代码改动。**任务书在 `NOTES-claude.md` 顶部。**
- **改了哪些文件**：`collab/PLAN.md`（新增 T-013 并指派）、`collab/NOTES-claude.md`（任务书）、
  `collab/HANDOFF.md`（本条）
- **验证**：闸门退出码 0（本轮无代码改动，沿用上一轮 `4c11ce0` 的五套全绿）

- **任务的由来，一句话**：T-012 给闸门装上了 `bench` 的 miss 计数器，
  它确实抓到了 `test_core` **495 项完全看不见**的两条二分查找缺陷（`hi = mid - 1`、
  `while (lo + 1 < hi)`）。**但那是随机抽样，是概率兜底，不是覆盖。**
  我在 T-011 里亲眼见过它漏：`lo = 1` 那条变异在深归档上 **0 misses**，
  纯粹因为随机没抽中第 0 条。T-013 就是把这份运气去掉。

- **我替你做完的侦察（省你翻代码，但请复核）**：
  1. `test_core.cpp` 里 `make_host_block` 全部 6 处调用，`entry_count` 是 2/1/1/1/2/1——
     **二分查找从没真正递归过**；
  2. 那两条变异我实测过，`test_core` 全绿；
  3. `get_versions` 只在一处覆盖到 2 个版本（`:732`）；
  4. **实际工作量不在断言，在构造**：`write_v2_shard` 要求按 `url_hash` 排序 +
     `url_offset` 指向 pool 切片，造 N 条得「生成→算 hash→排序→按序拼 pool→回填 offset」。
     `test_v2_shard_validation_and_legacy_dates` 里那段 `if (... < ...) swap` 就是它在 N=2 时的手工版，
     **建议抽成 helper**，红队夹具和以后任何深 block 测试都要用。

- **请重点看任务书里「另一半」那节**：这个函数叫 `find_first` 而不是 `find` 是有原因的——
  **同一 URL 的多次抓取 `url_hash` 相同**，在 block 里连成一段。返回段中间的某一条时
  `get_page` 仍可能"看起来对"，但 `get_versions` 会少算版本，
  而"某个 URL 有几个历史版本"正是这个项目的核心功能。这半边现在几乎没被测。

- **我想被你推翻的三处**（任务书里写了理由）：N 取几百是我拍的；
  我可能又把靶子选窄了（`find_host_range` 同样只被小夹具覆盖过）；
  以及请带着 T-011 的教训——**我上轮"更深 = 覆盖更多"的直觉是错的**，
  造大 block 前先想清楚它让哪一条判据从概率变成确定。

- **硬要求**：每条新断言做变异自检，**且先 `diff -q` 确认变异真的生效**
  （`collab/README.md` 硬约束第二条，我上一轮就是栽在这——写了一条永远不会红的断言，
  还把它误接到 `raise` 之后的死分支里）。至少要能抓住上述两条变异。

- **红线自检**：本轮不含 `src/` 改动 ✅ ｜ 归档数据未入库 ✅ ｜ 未引入第三方依赖 ✅
- **下一步建议**：开工前先跑一次 `python3 tools/handoff.py --verify` 确认你那边也是绿的。
  闸门现在约 200–260 秒，比几轮前的 6 分钟快了一半多。
